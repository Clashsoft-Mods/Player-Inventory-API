...
public class GuiCreativeInventory extends InventoryEffectRenderer
{
	...
	protected void renderInventoryTab(int mouseX, int mouseY, float partialTickTime)
	{
		...
		// Background Frame
		this.drawBackgroundFrame(x0, y0, windowWidth, windowHeight);
		
		GL11.glTranslatef(x0, y0, 0);
		
		// Player
		this.drawPlayerBackground(playerDisplayX, playerDisplayY);
		GuiSurvivalInventory.drawPlayerOnGui(this.mc, playerDisplayX + 16, playerDisplayY + 41, 20, x0 + playerDisplayX + 16 - mouseX, y0 + playerDisplayY + 10 - mouseY);
		...
		this.drawSlot(binSlotX, binSlotY, true);
	...
	protected void handleMouseClick(Slot slot, int slotID, int var1, int var2)
	{
		...
		else
		{
			...
			else
			{
				...
				this.mc.playerController.sendSlotPacket(itemstack, slot.slotNumber - this.inventorySlots.inventorySlots.size() + 9 + 36);
			}
		}
	}
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY)
	{
		...
		if (this.func_146978_c(playerDisplayX, playerDisplayY, 34, 45, mouseX, mouseY))
		{
			this.drawHoveringText(GuiSurvivalInventory.getPlayerInfo(this.player), mouseX - this.guiLeft, mouseX - this.guiTop, this.fontRendererObj);
		}
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTickTime)
	{
		...
		if (this.maxPages != 0)
		{
			...
			this.width = this.fontRendererObj.getStringWidth(page);
			GL11.glDisable(2896);
			this.zLevel = 300.0F;
			itemRender.zLevel = 300.0F;
			this.fontRendererObj.drawString(page, this.guiLeft + this.xSize / 2 - (this.width / 2), this.guiTop - 44, -1);
			this.zLevel = 0.0F;
